...
__all__ = [
    'featscale_cait_M48', 'featscale_cait_M36', 'featscale_cait_M4',
    'featscale_cait_S36', 'featscale_cait_S24','featscale_cait_S24_224',
    'featscale_cait_XS24','featscale_cait_XXS24','featscale_cait_XXS24_224',
    'featscale_cait_XXS36','featscale_cait_XXS36_224'
]
...
class LayerScale_Block_CA(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 Mlp_block=Mlp,init_values=1e-4, featscale=True):
        super().__init__()
    ...
    def forward(self, x, x_cls):
        ...
        x_attn = self.attn(self.norm1(u))
        if self.featscale:
            x_d, x_h = self.freq_decompose(x_attn)
            x_d = x_d * self.lamb1
            x_h = x_h * self.lamb2
            x_attn = x_attn + x_d + x_h        
        x_cls = x_cls + self.drop_path(self.gamma_1 * x_attn)
...
class LayerScale_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.featscale = featscale
        if self.featscale:
            self.lamb1 = nn.Parameter(torch.zeros(dim)) 
            self.lamb2 = nn.Parameter(torch.zeros(dim))
...
class cait_models(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 block_layers = partial(LayerScale_Block, featscale=True),
                 block_layers_token = partial(LayerScale_Block_CA, featscale=False),
...
@register_model
def featscale_cait_XXS24_224(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_XXS24(pretrained=False, **kwargs):
    model = cait_models(
    ...
    return model 
@register_model
def featscale_cait_XXS36_224(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_XXS36(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_XS24(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_S24_224(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_S24(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_S36(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_M36(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def featscale_cait_M48(pretrained=False, **kwargs):
    model = cait_models(
